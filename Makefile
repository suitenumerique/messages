...
mobile-ota-publish: ## build and publish a mobile OTA bundle (VERSION defaults to the short sha, CHANNEL to MOBILE_OTA_CHANNEL)
	@$(COMPOSE) up -d objectstorage --wait
	@$(COMPOSE_RUN) -e SOURCE_VERSION=$(MOBILE_OTA_BUILD_ID) frontend-mobile sh -c "npm run build && npm run mobile:ota:publish -- --version $(VERSION)$(if $(CHANNEL), --channel $(CHANNEL))"
...
build-front: ## build the frontend locally
	@$(COMPOSE) run --rm --build -e SOURCE_VERSION=$(MOBILE_OTA_BUILD_ID) frontend-tools npm run build
.PHONY: build-front
...
# Computed on the HOST (git is not in the container, and the bind mount carries
# no .git) and injected; CI may override it. The same value is also passed as
# SOURCE_VERSION to every container build, so the bundle's `__SOURCE_VERSION__`
# stamp (vite.config.ts, shown in the in-app support report) is the commit
# instead of the timestamp fallback — the id then matches the OTA manifest's.
...
mobile-build: ## build the web bundle and sync it + native plugins into the projects (container, env-aware)
	@$(COMPOSE) run --rm --build -e MOBILE_OTA_BUILD_ID=$(MOBILE_OTA_BUILD_ID) -e SOURCE_VERSION=$(MOBILE_OTA_BUILD_ID) -e MOBILE_VERSION_CODE=$(MOBILE_VERSION_CODE) frontend-mobile npm run mobile:build
.PHONY: mobile-build
...
# `sequence`, see docs/mobile-release.md). The displayed version (versionName / iOS
...
# (docs/mobile-release.md, App versioning).
...
mobile-android-release: mobile-build ## (host) build the signed Play bundle (.aab) — see docs/mobile-release.md
	@cd src/frontend/android && \
		MOBILE_APP_ID="$(call mobile_env,MOBILE_APP_ID)" \
		MOBILE_APP_NAME="$(call mobile_env,MOBILE_APP_NAME)" \
		MOBILE_AUTH_SCHEME="$(call mobile_env,MOBILE_AUTH_SCHEME)" \
