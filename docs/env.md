...
| `MOBILE_OTA_SIGNING_PRIVATE_KEY_B64` | None | Base64-encoded (single-line) RSA private-key PEM signing/encrypting each bundle at publish time (`publish-ota.mjs`). CI secret only — never commit. Unset ⇒ publish fails. See [mobile-release.md](./mobile-release.md#generating-the-signing-key-pair) | Optional |
...
`deploy/env/`. See [mobile-release.md](./mobile-release.md#publishing-to-google-play).
