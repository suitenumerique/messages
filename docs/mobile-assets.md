...
It also `require()`s packages it does not declare (`kleur`,
`@ionic/utils-terminal`, `@ionic/utils-subprocess`,
`@ionic/cli-framework-output`). With the linked install strategy
(`src/frontend/.npmrc`) a package only sees its declared dependencies and the
project's direct ones, so those four are declared in the frontend
`devDependencies` for that sole purpose — keep them in step with the versions
`@capacitor/cli` pulls, and do not work around a new missing one in the script.
