...
- **Task Workers**: Asynchronous task processing for heavy operations
...
- **MTA-Out (Outbound)**: worker-based SMTP delivery (direct to MX or via relay), with optional SOCKS proxying
...
- **Redis**: Caching layer and background-task broker
...
4. **Background tasks** processes sending via **MTA-Out** if `MTA_OUT_MODE=relay`, or directly to MX servers if `MTA_OUT_MODE=direct`
...
2. Backend queues indexing tasks to the task queue
3. task workers asynchronously index content in OpenSearch
4. Heavy operations (bulk imports, reindexing) are handled via background tasks
...
- **Async Processing**: Non-blocking operations via background tasks
...
- **Horizontal scaling** capability for backend and task workers
