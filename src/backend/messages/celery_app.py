...
if not settings.DISABLE_CELERY_BEAT_SCHEDULE:
    app.conf.beat_schedule = {
        ...
        "cleanup-trashbin": {
            # Permanently delete trashbin items (trashed + spam) older than
            # TRASHBIN_CUTOFF_DAYS. Daily housekeeping, default queue like the
            # other GC sweeps.
            "task": "core.services.trashbin.cleanup_trashbin_task",
        ...
        "gc-orphan-blobs": {
            ...
            # via ``python manage.py run_task core.services.blob_gc.gc_orphan_blobs_task --kwargs '{"mode": "full"}'``
