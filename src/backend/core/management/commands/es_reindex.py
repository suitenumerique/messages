...
class Command(BaseCommand):
    ...
    def _reindex_mailbox(self, mailbox_id):
        ...
        self.stdout.write(f"Reindexing threads in mailbox {mailbox.email}...")

        threads = models.Thread.objects.filter(mailbox_id=mailbox_uuid)
        total = threads.count()

        if total == 0:
            self.stdout.write(
                self.style.WARNING(f"No threads found in mailbox {mailbox.email}")
            )
            return

        self.stdout.write(
            f"Found {total} threads to reindex in mailbox {mailbox.email}"
        )
        success_count = 0
        failure_count = 0
