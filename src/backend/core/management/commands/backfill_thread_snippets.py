...
* the keyset scan, served by the ``thread_snippet_backfill_idx`` partial index
  (migration 0035) instead of one sequential scan per batch;
...
Each batch prints the keyset cursor it reached; pass it back as
``--resume-from`` to continue a run that was interrupted, or to step over
threads that legitimately keep ``snippet=""`` (draft-only threads, bodyless
messages, unreadable blobs) instead of re-reading their bodies on every
subsequent run. ``--before`` fences the pre-deploy backlog the same way, by
creation date.
...
derives nothing for it) — and this command cannot fill them either, so they
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Keyset cursor over ``(created_at, id)``, walked descending — the
        # index is scanned backward, so the ascending partial index serves
        # both directions.
        ...
        with (
            ...
            while scanned < limit and not stop.requested:
                ...
                if last_ct is not None:
                    ...
                    # PostgreSQL can only turn a leading conjunct into an index
                    # range bound: given the pure OR form it starts at the end
                    # of the index and *filters*, so every batch re-walks all
                    # the entries the cursor has already passed — quadratic
                    # over a full backfill. With ``created_at <= ct`` hoisted
                    # out, that becomes an Index Cond seek and the OR only
                    # discards the handful of rows sharing ``ct``. Measured on
                    # 300k rows at mid-scan (ascending variant, same plan
                    # shape): 14 buffers instead of 2886.
                ...
                # ``bulk_update`` in one write per batch, NOT per-row
                # ``save()``: signal-free, so the backlog is backfilled without
                # firing one search reindex per thread.
                if not dry_run and to_update:
                    models.Thread.objects.bulk_update(to_update, ["snippet"])
