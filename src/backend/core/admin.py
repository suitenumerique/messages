...
class MailboxAccessAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        pre-save snapshot rather than the new ``obj``.
        """
        previous = None
    ...
    def delete_queryset(self, request, queryset):
        ...
        memory afterwards for the cleanup pass.
        """
        rows = list(queryset)
...
def _cleanup_thread_access_formset(formset):
    ...
    revokes the old grant rather than leaving it behind.
    """
    previous_snapshots = {}
    instances_to_revoke = []
    ...
    for sub_form in formset.forms:
        ...
        if sub_form.cleaned_data.get("DELETE"):
            instances_to_revoke.append(sub_form.instance)
            continue
...
def _cleanup_mailbox_access_formset(formset):
    ...
    leaving stale assignment/user-event state behind.
    """
    previous_snapshots = {}
    instances_to_revoke = []
