...
from core.models import parse_max_recipients
    ...
    @extend_schema_field(
        {
            ...
            "properties": {
                ...
                "max_recipients": {
                    ...
                    "nullable": True,
                    "description": (
                        "Maximum recipients per period "
                        "(format: 'number/period', e.g., '500/d' for 500 per day)."
                    ),
                },
            },
            "nullable": True,
...
class CustomSettingsUpdateMixin:
    ...
    def validate_custom_settings(self, value):
        ...
        if value and isinstance(value, dict):
            ...
            max_recipients_per_msg = value.get("max_recipients_per_message")
            if max_recipients_per_msg is not None and isinstance(
                max_recipients_per_msg, int
            ):
                if max_recipients_per_msg <= 0:
                    raise serializers.ValidationError(
                        _("The limit must be greater than 0.")
                    )
                if max_recipients_per_msg > settings.MAX_RECIPIENTS_PER_MESSAGE:
                    raise serializers.ValidationError(
                        _(
            ...
            max_recipients = value.get("max_recipients")
            if max_recipients is not None:
                try:
                    limit, period = parse_max_recipients(max_recipients)

                    # Determine which max to use based on model type
                    # For mailboxes, use MAX_RECIPIENTS_FOR_MAILBOX
                    # For domains, use MAX_RECIPIENTS_FOR_DOMAIN
                    if self.Meta.model == models.Mailbox:
                        max_setting = settings.MAX_RECIPIENTS_FOR_MAILBOX
                    else:
                        max_setting = settings.MAX_RECIPIENTS_FOR_DOMAIN

                    global_limit, global_period = parse_max_recipients(max_setting)

                    # Only compare if periods match
                    if period == global_period and limit > global_limit:
                        raise serializers.ValidationError(
                            _("The limit cannot exceed the global maximum of %(max)s.")
                            % {"max": max_setting}
                        )

                    # Note: With Redis-based quotas, mailbox and domain quotas are
                    # independent. Domain capacity constraint is enforced at send time,
                    # not at configuration time.

                except ValueError:
                    raise serializers.ValidationError(
                        _("Invalid format for recipients limit.")
                    ) from None
...
class RecipientQuotaSerializer(serializers.Serializer):
    """Serializer for recipient quota status (read-only)."""

    period = serializers.CharField(help_text="The quota period type (d=day, m=month)")
    period_display = serializers.CharField(help_text="Human-readable period name")
    period_start = serializers.DateTimeField(
        help_text="Start of the current quota period"
    )
    recipient_count = serializers.IntegerField(
        help_text="Number of recipients sent during this period"
    )
    quota_limit = serializers.IntegerField(
        help_text="Maximum number of recipients allowed during this period"
    )
    remaining = serializers.IntegerField(
        help_text="Number of remaining recipients that can be sent to"
    )
    usage_percentage = serializers.FloatField(
        help_text="Percentage of quota used (0-100)"
    )
    ...
    def create(self, validated_data):
        """This serializer is read-only, create/update are never called."""
        raise NotImplementedError("RecipientQuotaSerializer is read-only")
