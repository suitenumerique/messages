...
from core.management.commands import backfill_thread_snippets as backfill_command
...
class TestBackfillThreadSnippets:
    ...
    def _isolate_cursor(self):
        """The cached keyset cursor outlives a run by design — and a test."""
        cache.delete(backfill_command._CURSOR_CACHE_KEY)
        yield
        cache.delete(backfill_command._CURSOR_CACHE_KEY)
    ...
    def test_resume_cursor_is_printed_and_skips_past(self):
        ...
        already scanned. Runs carry it over on their own via the cache; this
        is the manual override, for stepping a run forward by hand."""
        older = _dormant_thread()
        newer = _dormant_thread()
    ...
    def test_cursor_survives_the_run_and_skips_what_it_walked(self):
        """A second run starts where the first stopped.

        Regression: threads that legitimately keep ``snippet=""`` — unreadable
        blobs, bodyless messages, draft-only threads — never leave the
        queryset. Without a cursor outliving the run they pile up at the head
        of the scan, and every cron run re-reads their blobs before reaching a
        single fillable thread.
        """
        thread = _dormant_thread()
        ...
        out = StringIO()
        with patch.object(models.Message, "get_parsed_data", unreadable):
            call_command("backfill_thread_snippets", stdout=out)
        assert "scanned=1 " in out.getvalue()
        thread.refresh_from_db()
        assert thread.snippet == ""

        # The blob reads fine now, but the thread sits behind the cursor.
        out = StringIO()
        call_command("backfill_thread_snippets", stdout=out)
    ...
    def test_restart_rescans_from_the_top(self):
        """--restart drops the cursor, so a stepped-over thread is retried."""
        thread = _dormant_thread()
        ...
        out = StringIO()
        call_command("backfill_thread_snippets", "--restart", stdout=out)

        assert "scanned=1 " in out.getvalue()
        thread.refresh_from_db()
        assert thread.snippet == "Backfilled body content"

    def test_dry_run_leaves_the_cursor_untouched(self):
        """A rehearsal must not make the next real run skip what it walked."""
        thread = _dormant_thread()

        call_command("backfill_thread_snippets", "--dry-run", stdout=StringIO())
    ...
    def test_reset_clears_the_cursor(self):
        """The purge refills the backlog behind the cursor, so it must drop it.

        Otherwise every thread the cursor already walked past stays empty for
        good — the rebuild run would start below them.
        """
        thread = _dormant_thread()
        call_command("backfill_thread_snippets", stdout=StringIO())
        thread.refresh_from_db()
        ...
        call_command("backfill_thread_snippets", "--reset", stdout=StringIO())
    ...
    def test_live_snippet_is_not_overwritten_by_a_stale_backfill(self):
        """A snippet written while the batch was reading bodies survives.

        Regression: the batch selects on ``snippet=""``, then spends seconds
        reading blobs. A message landing meanwhile has ``update_stats`` derive
        a fresher snippet, and the unguarded write replaced it with the one
        derived from the message that was latest when the batch started —
        permanently, since the thread has since left the queryset.
        """
        thread = _dormant_thread()
        original = backfill_command.Command._release_content

        def racing(chunk):
            # Live activity lands after the snippet was derived, before the
            # batch writes. Injected here rather than in ``_derive``: that one
            # runs on the worker pool, where the ORM is off-limits.
            models.Thread.objects.filter(id=thread.id).update(snippet="Live snippet")
            original(chunk)

        out = StringIO()
        with patch.object(
            backfill_command.Command, "_release_content", staticmethod(racing)
        ):
            call_command("backfill_thread_snippets", stdout=out)

        thread.refresh_from_db()
        assert thread.snippet == "Live snippet"
        # The counter reports rows actually written, not snippets derived.
        assert "populated=0 " in out.getvalue()
