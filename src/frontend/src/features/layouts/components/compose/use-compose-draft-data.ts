...
type UseComposeDraftDataResult = {
    ...
    isLoading: boolean;
    /**
     * The draft can no longer be edited: it or its parent was deleted or made
     * inaccessible elsewhere (404), or it was already sent from another
     * surface (the message still exists, but as a sent one).
     */
    isDraftNotFound: boolean;
    /**
     * Something the form needs failed to load for a reason worth retrying
     * (403, 5xx, offline). Distinct from `isDraftNotFound`: the draft may well
     * still be there.
     */
    loadError: boolean;
    retry: () => void;
...
export const useComposeDraftData = ({
    ...
    const fetchedDraft = draftQuery.data?.data;

    const effectiveParentId = parentMessageId ?? fetchedDraft?.parent_id ?? undefined;
    ...
    const effectiveThreadId = effectiveParentId ? (threadId ?? fetchedDraft?.thread_id ?? undefined) : undefined;
    ...
    // A persisted window may outlive its draft: sent from another tab or
    // client, the id still resolves (200) but to a message that must never be
    // mounted in an editor. A reply whose parent is gone is no better off.
    const isDraftNotFound =
        isNotFoundError(draftQuery.error)
        || (!!fetchedDraft && !fetchedDraft.is_draft)
        || (!!effectiveParentId && isNotFoundError(parentQuery.error));
    const draft = isDraftNotFound ? undefined : fetchedDraft;

    const loadError =
        !isDraftNotFound
        && (draftQuery.isError
            || (!!effectiveParentId && parentQuery.isError)
            || (!!effectiveThreadId && threadQuery.isError));
    const retry = () => {
        if (draftQuery.isError) void draftQuery.refetch();
        if (parentQuery.isError) void parentQuery.refetch();
        if (threadQuery.isError) void threadQuery.refetch();
    };
    ...
    const isLoading =
        ...
        && (
            ...
            || (!!effectiveParentId && !parentMessage)
            || (!!effectiveThreadId && !threadQuery.data)
        );

    return { mailbox, draft, parentMessage, thread, isLoading, isDraftNotFound, loadError, retry };
};
