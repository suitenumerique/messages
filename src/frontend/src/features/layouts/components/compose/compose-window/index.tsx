...
export const ComposeWindow = ({ descriptor, isOverflowed = false }: ComposeWindowProps) => {
    ...
    const isModal = isFloating || isSheet;
    ...
    const senderMailbox = getSender(descriptor.mailboxId);
    // The docked tab is too narrow to spell the address out, and the avatar
    // that stands in for it is decorative, so the sender only reaches both the
    // tooltip and assistive tech through this name. Keeping `title` as a
    // suffix matters: the visible text must remain part of the accessible
    // name, or voice control can no longer target the tab.
    ...
    const requestSend = () => formRef.current?.requestSend();
    // With `onTap` the hook settles tap vs long press from the touch sequence
    // itself, so a long press never sends and a tap sends exactly once.
    const { handlers: sendLongPressHandlers, isTouchHandled: isSendTouchHandled } = useLongPress(
        () => {
            if (canArchiveOnSend) openSendMenu();
        },
        { onTap: requestSend },
    );

    const handleSendClick = () => {
        // Touch already sent from `onTap`; this is only the compatibility
        // click a browser emitted anyway.
        if (isSendTouchHandled()) return;
        requestSend();
    };

    // Keyboard (Shift+F10, Menu key) and mouse equivalent of the long press.
    const handleSendContextMenu = (event: React.MouseEvent) => {
    ...
    useFocusTrap(rootRef, isModal);
    // Closing hands the focus back to whatever opened the window (the "New
    // message" button, a reply action…). Minimizing keeps the window mounted,
    // hence the dedicated effect below.
    useRestoreFocus();
    ...
    // Minimizing hides the body, which drops the focus it held to <body>:
    // land it on the dock tab instead, so the keyboard user stays where the
    // window went. Only on the transition — restored windows mount minimized
    // and must not grab the focus on page load. On mobile the tab is hidden
    // too and focus() is a no-op there; the stack bar takes over
    // (see ComposeWindowsLayer).
    useEffect(() => {
            ...
            >
                ...
                >
                    ...
                    <button
                        ...
                        title={titleWithSender}
                        aria-label={titleWithSender}
                        onClick={toggleMinimize}
                    >
                        {senderMailbox && (
                            <span
                                className="compose-window__title-avatar"
                                data-shared={!senderMailbox.is_identity}
                                aria-hidden="true"
                            >
                                <UserAvatar
                                    fullName={MailboxHelper.getLabel(senderMailbox)}
                                    size="xsmall"
                                />
                            </span>
                        )}
                        <span className="compose-window__title-text">{title}</span>
                    </button>
                    {isSheet && (
                        <DropdownMenu
                        ...
                        >
                            <Tooltip content={sheetSendMode === PreferSendMode.SEND_AND_ARCHIVE ? t("Send and archive") : t("Send")}>
                                <Button
                                    ...
                                    aria-haspopup={canArchiveOnSend ? "menu" : undefined}
                                    aria-expanded={canArchiveOnSend ? isSendMenuOpen : undefined}
                        ...
                        <div className="compose-window__actions">
                            {!isMinimized && !isSheet && (
                                <>
                                    ...
                                    <Tooltip content={isFloating ? t("Exit full-screen mode") : t("Full-screen")}>
                                        <Button
                                            type="button"
                                            color="neutral"
                                            variant="tertiary"
                                            size="small"
                                            aria-label={isFloating ? t("Exit full-screen mode") : t("Full-screen")}
