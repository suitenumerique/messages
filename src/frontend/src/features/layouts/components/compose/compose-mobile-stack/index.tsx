...
type ComposeMobileStackProps = {
    ...
    onOpenOverview: () => void;
    /** Take the focus on mount: the bar replaces a sheet that held it. */
    autoFocus?: boolean;
};
 ...
 */
export const ComposeMobileStack = ({ windows, onOpenOverview, autoFocus = false }: ComposeMobileStackProps) => {
    ...
    useEffect(() => {
        if (autoFocus) barRef.current?.focus();
    }, [autoFocus]);
