...
import { Button, Checkbox, Tooltip } from "@gouvfr-lasuite/ui-components"
import { IconSize, IconType, UserAvatar } from "@gouvfr-lasuite/ui-components"
...
import { AttachFile, Edit, Star, StarFilled } from "@gouvfr-lasuite/ui-components/icons"
...
export const ThreadItem = ({ thread, isSelected, onToggle, onSelectRange, selectedThreadIds, isSelectionMode, tabIndex, itemRef, onFocusItem }: ThreadItemProps) => {
    ...
    const { handlers: longPressHandlers } = useLongPress(() => {
        ...
        // The row is also swipeable: the press must give up exactly where
        // the swipe claims the gesture, or a slow diagonal drag ends up
        // both revealing the actions and entering selection mode.
        moveTolerance: AXIS_LOCK_THRESHOLD,
    });
                ...
                <div>
                    ...
                    <div className="thread-item__row thread-item__row--subject">
                        <div className="thread-item__column">
                            <Link
                                ...
                                aria-describedby={`${dateId} ${badgesId}${thread.labels.length > 0 ? ` ${labelsId}` : ""}`}
                                onFocus={onFocusItem}
                                tabIndex={tabIndex}
                                ref={itemRef}
                                role="option"
                        ...
                        <div className="thread-item__column thread-item__column--badges">
                            {/* Only statuses describe the option; the star action has its own accessible name. */}
                            <div className="thread-item__status-badges" id={badgesId}>
                                {thread.has_draft && (
                                ...
                                {!canToggleStar && thread.has_starred && (
                    ...
                    <div className="thread-item__row">
                        {thread.labels.length > 0 && (
                            <div className="thread-item__labels" id={labelsId}>
