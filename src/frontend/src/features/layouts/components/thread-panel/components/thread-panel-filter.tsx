...
export const ThreadPanelFilter = () => {
  ...
  if (isNative) {
    // The keyboard (Shift+F10, Menu key) and mouse equivalent of the long
    // press: a native shell can have a hardware keyboard too. Android's own
    // contextmenu for the long press never lands here (see useLongPress).
    const handleContextMenu = (event: React.MouseEvent<HTMLElement>) => {
      event.preventDefault();
      const rect = event.currentTarget.getBoundingClientRect();
      // A keyboard-summoned event carries no pointer position.
      const position =
