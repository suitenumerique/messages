...
import { isTerminal } from "@/hooks/use-import-status";
...
/**
 * Same strategy as the auto-reply indicator: while at least one import run is
 * in progress for the selected mailbox, show a live progress button in the
 * header that opens the mailbox settings modal on the Imports tab — several
 * imports can run at once, so the old single-task tracking in the gear menu is
 * no longer enough to follow them.
 */
