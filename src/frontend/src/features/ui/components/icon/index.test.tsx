...
describe("Icon", () => {
    ...
    it("keeps the same SVG node across re-renders", async () => {
        // A rebuilt SVG would strand any touch that started on it: touch
        // events stay bound to the node they began on, even once detached.
        const container = document.createElement("div");
        const root = createRoot(container);
        const render = () =>
            act(async () => {
                root.render(<Icon name="reply" size={16} />);
            });
        await render();
        for (let i = 0; i < 20 && !container.querySelector("svg"); i += 1) {
            await act(async () => {
    ...
    it("proxies ui-kit SVG icon components through the icon prop", () => {
        ...
        const decorative = renderToStaticMarkup(
            <Icon icon={UIKitSvgIcon} size={32} color="red" className="custom" />,
        );
        expect(decorative).toContain('data-size="32"');
        expect(decorative).toContain('data-color="red"');
        expect(decorative).toContain('class="messages-icon messages-icon--auto-size custom"');
    ...
    it("makes unsized ui-kit SVG icons follow the font-size context", () => {
        ...
        expect(html).toContain('class="messages-icon"');
