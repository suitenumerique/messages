...
export const Icon = (props: IconProps) => {
    ...
    if (props.icon) {
        ...
        const emSizing = size === undefined ?
        { width: "1em", height: "1em" }
        : { className: clsx("messages-icon", 'messages-icon--auto-size', className ) };
        return (
            <SvgIcon
                size={size}
                color={color}
...
const AppIcon = ({
    ...
    // React 19 diffs `dangerouslySetInnerHTML` by object identity: a fresh
    // `{ __html }` reassigns `innerHTML` on every render, rebuilding the SVG.
    // A touch sequence stays bound to the node it started on, so an SVG
    // replaced under the finger (a `pressing` state flip on `touchstart`…)
    // takes its `touchend` away from React and turns the tap into a long press.
    const innerHtml = useMemo(
        () => (markup ? { __html: markup } : undefined),
        [markup],
    );
