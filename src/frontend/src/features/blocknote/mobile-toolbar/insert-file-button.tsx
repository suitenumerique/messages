...
/**
 * The "insert a file" toolbar option: a popover choosing between an inline
 * image and a plain attachment. Portaled to <body> because the toolbar row
 * is a horizontal scroller whose overflow would clip anything anchored
 * inside it; the coordinates are measured from the button on open (the bar
 * is position: fixed with no transform, so viewport coordinates are stable
 * while the popover is up).
 *
 * Not an ARIA menu: that pattern moves the focus into the list and drives it
 * with the arrow keys, while this whole toolbar keeps the focus in the editor
 * on purpose (the keyboard would fold and tear the bar down otherwise). So it
 * is announced for what it is — a labelled group of buttons the trigger
 * expands — and closes on Escape, backdrop tap, or a pick.
 */
export const InsertFileButton = ({
    ...
    // The focus stays in the editor while the popover is up, so Escape has
    // to be caught at the document level (external keyboard).
    useEffect(() => {
        if (!isOpen) return;
