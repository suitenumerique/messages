...
export const MobileToolbar = ({
    ...
    // The link editor snapshots the link under the caret when it mounts, so
    // each opening gets a fresh instance: tapping another link while the
    // editor is already up must not keep showing the first link's URL.
    const [linkEditorSession, setLinkEditorSession] = useState(0);
    // Tapping a link edits it instead of navigating (the default link
    // toolbar is disabled on native, see BlockNoteViewField).
    const openLinkEditor = useCallback(() => {
        setView("link");
        setLinkEditorSession((session) => session + 1);
    }, []);
    ...
    // Stable on purpose: owners bind it to their unmount, and a new identity
    // on every open would run that cleanup — and close the drawer — right as
    // it opens.
    const releaseChildDrawer = useCallback((id: string) => {
        setChildDrawerId((current) => (current === id ? null : current));
    }, []);
    ...
    const drawerApi = useMemo(
        ...
        [drawerSlot, childDrawerId, editor, releaseChildDrawer],
    );
    ...
    return createPortal(
        ...
        >
            ...
            {view === "link" && (
                <MobileLinkEditor key={linkEditorSession} onClose={() => setView("toolbar")} />
            )}
