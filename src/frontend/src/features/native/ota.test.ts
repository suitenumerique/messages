...
const primeUpdate = (
  ...
  // Signed by default: every scenario below is about ordering/guards, and an
  // unsigned manifest is refused before any of those run (see its own tests).
  ctx.http.get.mockResolvedValue({
    data: { url: "http://ota.test/bundle.zip", checksum: "chk", sessionKey: "sk", ...manifest },
  });
...
describe("checkAndStageOtaUpdate", () => {
  ...
  it.each([
    ["without checksum", { checksum: undefined }],
    ["without sessionKey", { sessionKey: undefined }],
    ["with an empty checksum", { checksum: "" }],
    ["with an empty sessionKey", { sessionKey: "" }],
  ])("refuses a manifest %s (unsigned bundle)", async (_label, unsigned) => {
    const ctx = await loadOta();
    const consoleWarn = vi.spyOn(console, "warn").mockImplementation(() => {});
    primeUpdate(ctx, {
      current: "100-aaa",
      manifest: { version: "101-bbb", sequence: 7, ...unsigned },
    });
    await ctx.ota.checkAndStageOtaUpdate(MANIFEST_URL);
    // Nothing reaches the native layer — download() is where Android would
    // otherwise accept a plain checksum in lieu of a signature.
    expect(ctx.updater.download).not.toHaveBeenCalled();
    expect(ctx.updater.next).not.toHaveBeenCalled();
    // A refused manifest must not move the anti-replay floor either.
    expect(localStorage.getItem(SEQUENCE_KEY)).toBeNull();
    expect(consoleWarn).toHaveBeenCalledWith(
      `OTA manifest at ${MANIFEST_URL} carries no checksum/sessionKey; ` +
        "refusing to stage an unverifiable bundle.",
    );
  });

  // Vitest runs as a dev build (import.meta.env.DEV), where cleartext is
  // tolerated for the local bucket; the guard only bites in a release build.
  it("refuses a plain-http manifest URL in a release build", async () => {
    const ctx = await loadOta();
    vi.stubEnv("DEV", false);
    const consoleWarn = vi.spyOn(console, "warn").mockImplementation(() => {});
    primeUpdate(ctx, { current: "100-aaa", manifest: { version: "101-bbb", sequence: 7 } });
    await ctx.ota.checkAndStageOtaUpdate(MANIFEST_URL);
    expect(ctx.http.get).not.toHaveBeenCalled();
    expect(consoleWarn).toHaveBeenCalledWith(
      `OTA manifest URL ${MANIFEST_URL} is not https; refusing to fetch it ` +
        "outside a dev build.",
    );
  });

  it("refuses a plain-http bundle URL in a release build", async () => {
    const ctx = await loadOta();
    vi.stubEnv("DEV", false);
    const consoleWarn = vi.spyOn(console, "warn").mockImplementation(() => {});
    primeUpdate(ctx, {
      current: "100-aaa",
      manifest: { version: "101-bbb", sequence: 7, url: "http://ota.test/bundle.zip" },
    });
    await ctx.ota.checkAndStageOtaUpdate("https://ota.test/channels/prod/manifest.json");
    expect(ctx.updater.download).not.toHaveBeenCalled();
    expect(ctx.updater.next).not.toHaveBeenCalled();
    expect(localStorage.getItem(SEQUENCE_KEY)).toBeNull();
    expect(consoleWarn).toHaveBeenCalledWith(
      "OTA bundle URL http://ota.test/bundle.zip is not https; refusing to download it " +
        "outside a dev build.",
    );
  });

  it("keeps cleartext URLs for a dev build", async () => {
    const ctx = await loadOta();
    vi.stubEnv("DEV", true);
    primeUpdate(ctx, { current: "100-aaa", manifest: { version: "101-bbb", sequence: 7 } });
    await ctx.ota.checkAndStageOtaUpdate(MANIFEST_URL);
  ...
  it("refuses an unsigned manifest even for a locally installed copy", async () => {
    const ctx = await loadOta();
    vi.spyOn(console, "warn").mockImplementation(() => {});
    primeUpdate(ctx, {
      current: "100-aaa",
      manifest: { version: "101-bbb", checksum: undefined, sessionKey: undefined },
      localBundles: [{ id: "local-101", version: "101-bbb", status: "success" }],
    });
    await ctx.ota.checkAndStageOtaUpdate(MANIFEST_URL);
    expect(ctx.updater.next).not.toHaveBeenCalled();
  });
