...
import { getHeaders, getRequestUrl } from "@/features/api/utils";
import { APP_STORAGE_PREFIX } from "@/features/config/constants";

import { AuthSessionCancelledError } from "./auth-errors";
...
export const nativeLogin = async (): Promise<void> => {
  try {
    ...
    clearPendingLogin();
    // A dismissed system-browser sheet is the user's own doing: stay on the
    // login screen quietly. Anything else (exchange refused, network) is a
    // failure the caller has to surface.
    if (error instanceof AuthSessionCancelledError) {
      console.warn("Native login was cancelled.");
      return;
    }
    throw error;
  }
