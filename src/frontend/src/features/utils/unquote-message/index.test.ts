...
describe("UnquoteMessage", () => {
  describe("HTML - Handler-based quote removal", () => {
    it("should handle multiple quote markers", () => {
      ...
      expect(result.content).not.toContain("First quote");
      expect(result.content).not.toContain("Second quote");
      expect(result.content).toContain("New message");
    });
    ...
    describe("Handlers - custom detect", () => {
      ...
      it("should remove Ymail signature and all following content", () => {
        ...
        expect(result.hadQuotes).toBe(true);
        expect(result.detectionMethod).toBe("handlers");
        expect(result.content).toContain("My message");
      ...
      it("should remove Zoho quotes and all following content", () => {
        ...
        expect(result.detectionMethod).toBe("handlers");
        expect(result.content).toContain("My message");
        expect(result.content).not.toContain("Quoted content 1");
        expect(result.content).not.toContain("Quoted content 2");
        expect(result.content).not.toContain("beforequote:::");
      });
      ...
      it("should remove Alimail quotes (alimail-quote inside blockquote)", () => {
        ...
        expect(result.detectionMethod).toBe("handlers");
        expect(result.content).toContain("New message");
        expect(result.content).not.toContain("Alimail quoted content");
      });
...
> Old quoted text
      ...
      expect(result).not.toContain("> Old quoted text");
      expect(result).toContain("New message");
    });
  ...
  describe("Complex real-world scenarios", () => {
    it("should handle Gmail-style reply with multiple elements", () => {
      ...
      expect(result.hadQuotes).toBe(true);
      expect(result.content).toContain("This is my reply");
      expect(result.content).not.toContain("Original message content");
    });
    ...
    it("should handle Outlook-style reply with hr separator", () => {
      ...
      expect(result.content).toContain("My reply to your message");
      // The handler should catch divRplyFwdMsg or the pattern should catch hr
      expect(result.hadQuotes).toBe(true);
    });
    ...
    it("should handle mixed quote types", () => {
      ...
      expect(result.hadQuotes).toBe(true);
      expect(result.content).not.toContain("First quote");
      expect(result.content).not.toContain("Nested Gmail quote");
    });
...
> > His message
      ...
      expect(result.content).not.toContain("Her message");
      expect(result.content).not.toContain("His message");
    });
  });
});
