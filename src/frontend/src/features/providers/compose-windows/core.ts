...
export const openWindowReducer = (windows: ComposeWindowDescriptor[], descriptor: ComposeWindowDescriptor): OpenWindowResult => {
    ...
    // is auto-filled, hence no title condition there). The mailbox is part of
    // the identity: a window opened from another mailbox would send from the
    // wrong address.
    if (!descriptor.draftId) {
        const duplicate = windows.find((window) =>
            !window.draftId
            && window.mailboxId === descriptor.mailboxId
