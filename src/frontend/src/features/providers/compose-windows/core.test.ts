...
describe("openWindowReducer", () => {
    ...
    it("opens a fresh window when the blank new-message one belongs to another mailbox", () => {
        const blank = makeWindow({ mode: "new", mailboxId: "mailbox-1" });
        const { windows, outcome } = openWindowReducer([blank], makeWindow({ mode: "new", mailboxId: "mailbox-2" }));
        expect(outcome).toBe("opened");
        expect(windows).toHaveLength(2);
        expect(windows[0].isMinimized).toBe(true);
        expect(windows[1].mailboxId).toBe("mailbox-2");
    });
