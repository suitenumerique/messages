...
import { isFocusMoveWithinToolbar } from "@/features/blocknote/toolbar-focus";
...
export const MessageForm = forwardRef<MessageFormHandle, MessageFormProps>(({
    ...
    // footer paperclip button opens the file picker. No `maxSize`: the hook
    // enforces the limit with feedback, see AttachmentUploader.
    ...
    /**
     * "Delete" on a form whose draft never showed up: the autosave may be
     * materializing it right now, and closing on the spot would leave that
     * draft behind as an orphan.
     */
    const handleDiscardUnsaved = async () => {
        await saveDraftPromiseRef.current;
        const id = draftRef.current?.id;
        if (id) await deleteDraftCore(id, { notify: false });
        else onClose?.();
    }
    ...
    useImperativeHandle(ref, () => ({
        deleteDraft: async () => {
            // The draft may still be materializing (first autosave in
            // flight): reading the id before it lands would leave an orphan.
            await saveDraftPromiseRef.current;
            const id = draftRef.current?.id;
     ...
     * redirect the user to the new draft view. Returns the new draft id.
     */
    const handleChangeSender = async (currentDraft: Message, data: DraftMessageRequestRequest): Promise<string> => {
        await deleteMessageMutation.mutateAsync({ id: currentDraft.id });
        ...
        const threadId = newDraft.thread_id;
        if (threadId) {
     ...
     * When `force` is true, save unconditionally: bypass the content and
     * dirtiness checks as well as the send lock (used by ensureDraft and by
     * the send flow, which awaits the save before firing).
        ...
        const promise = (async () => {
            ...
            try {
                ...
                if (!currentDraft) {
                    ...
                    return await handleChangeSender(currentDraft, payload);
                } else {
    ...
    /**
     * Ensure a draft exists server-side and reflects the form, creating or
     * flushing it if necessary. Returns the draft id.
     *
     * Callers hand the draft over to another surface (pop-out tab) which
     * loads it by id: an in-flight or pending save must land first, or that
     * surface starts from the pre-edit body and its own autosave writes it
     * back over the edits.
     */
    const ensureDraft = async (): Promise<string | undefined> => {
        // Without a draft, force the creation even for an empty form; with
        // one, the regular path awaits the running save or flushes the dirty
        // fields, and is a no-op when nothing changed.
        return saveDraftInner(!draftRef.current);
    }
    ...
    const handleSubmit = async ({ archive }: { archive: boolean }) => {
        ...
        try {
            // Wait for any in-progress draft save to complete. It may predate
            // the last edits, hence the unconditional flush right after.
            await saveDraftPromiseRef.current;

            // The send payload only carries the body: subject, recipients and
            // attachments are read server-side from the stored draft. Push the
            // form as-is (creating the draft if needed) rather than trusting a
            // blur-triggered save to have run: on touch devices, tapping Send
            // does not blur the field being edited, so a subject typed right
            // before sending never reached the server and the message left
            // without it.
            const messageId = await saveDraftInner(true);
            if (!messageId) {
                abortSend();
                return;
            }
    ...
    const formElementProps = {
        ...
        onBlur: (event: React.FocusEvent<HTMLFormElement>) => {
            // Tabbing across the composer toolbar is not a field blur: nothing
            // new to save, and the save's re-render would throw the focus back
            // to the toolbar's first button (see isFocusMoveWithinToolbar).
            if (isFocusMoveWithinToolbar(event)) return;
        ...
        <FormProvider {...form}>
            ...
            <form {...(compact ? { ...formDropzone.getRootProps(), ...formElementProps } : formElementProps)}>
                ...
                <footer className="form-footer" ref={footerRef}>
                    <div className="form-footer__column">
                    ...
                    {!draft && onClose && (
                        <Tooltip content={t("Delete")}>
                            <Button
                                ...
                                onClick={() => void handleDiscardUnsaved()}
                                aria-label={t("Delete")}
                    ...
                    <div className="form-footer__column">
                    <DraftSaveStatus
                        className="form-footer__save-time"
                        savedAt={draft?.updated_at}
                        isSaving={draftCreateMutation.isPending || draftUpdateMutation.isPending}
                        now={currentTime}
                    />
                    </div>
                </footer>
