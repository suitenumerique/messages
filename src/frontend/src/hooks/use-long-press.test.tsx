...
/** Element the press lands on; in the document so events reach `window`. */
let pressed: HTMLButtonElement;
...
const contextMenu = (target: EventTarget = pressed) => {
...
beforeEach(() => {
    ...
    pressed = document.createElement("button");
...
describe("useLongPress", () => {
    ...
    describe("browser's own long-press contextmenu", () => {
        it("swallows it while the press is held, before and after the delay", () => {
            touchStart(20, 40);
            wait(100);
        ...
        it("keeps it from reaching listeners further down the capture path", () => {
            const seen = vi.fn();
            document.addEventListener("contextmenu", seen, true);
            touchStart(20, 40);
            wait(350);
            contextMenu();
            document.removeEventListener("contextmenu", seen, true);
            expect(seen).not.toHaveBeenCalled();
        });

        it("leaves a right-click alone once the press is over", () => {
            touchStart(20, 40);
            wait(100);
            touchEnd();
        ...
        it("swallows it when it lands outside the pressed element", () => {
            // Once the long press opened a menu, its underlay sits under the
            // finger: the browser's hit test targets it, not the button.
            touchStart(20, 40);
            wait(350);
            expect(onLongPress).toHaveBeenCalledTimes(1);
            expect(contextMenu(container).defaultPrevented).toBe(true);
        });

        it("leaves a right-click alone when no press is in flight", () => {
            expect(contextMenu().defaultPrevented).toBe(false);
            expect(contextMenu(container).defaultPrevented).toBe(false);
        });
    });
