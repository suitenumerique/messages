...
import { compareVersionCounts } from "../src/features/native/ota-version.mjs";
import {
  bundleKey,
  computeNextSequence,
  manifestKey,
...
let sequence = null;
if (!stageOnly) {
  // Publish-side mirror of the client's *legacy* downgrade guard (see ota.ts):
  // a device still running a pre-sequence client orders releases by the hybrid
  // id's count and refuses a manifest that does not order above what it runs,
  // so publishing one could only strand it. Sequence-aware clients follow the
  // counter regardless, and sha ids cannot be ordered at all, so the guard
  // only bites on hybrid-era ids. Fail fast — before the zip/encrypt work —
  // unless it is an idempotent republish of the exact same version (e.g. a CI
  // retry).
  const existing = await readJson({ client, bucket }, channelManifestKey);
  if (existing && existing.version !== version && !values.force) {
    const order = compareVersionCounts(version, existing.version);
